...
- **🎨 Modern Fluent UI 极简设计、自定义背景与 MiSans 字体**
  ...
  - **🖼️ 自定义软件背景图**：自动遍历 `assets/background/` 目录，支持在设置中下拉选择图片与一键刷新。
  - **🎚️ 背景透明度控制滑块**：内置 QFluentWidgets `Slider` 控件，支持 10% - 100% 透明度实时调控。
  - **✨ 通透玻璃卡片 (Frosted Glass)**：左侧扫码区重构为单一体化悬浮玻璃卡片组，背景图可清爽透出。
  - **🔤 统一字体适配**：桌面端与 H5 移动页面全局优先使用 **MiSans**（小米兰亭字体），提供极致视觉呈现。
  - 支持 Windows 11 Mica 云母半透明材质效果，以及跟随系统 / 浅色模式 / 深色模式全局一键切换。
...
Photo/
├── assets/                   # 图标与背景图片资源目录
│   ├── background/           # 自定义背景图资源目录 (支持 .png, .jpg, .webp 等)
│   ├── lumalink_full_lockup.png
│   └── lumalink_icon_mark.png
├── main.py                   # 应用程序主入口
